...
{
	this->metric = new NearNeighborDisMetric(NearNeighborDisMetric::METRIC_TYPE::cosine,
		max_cosine_distance, nn_budget);
	this->max_iou_distance = max_iou_distance;
	this->max_age = max_age;
	this->n_init = n_init;

	this->kf = new KalmanFilterhc();
	this->tracks.clear();
	this->_next_idx = 1;
}
...
void tracker::update(const DETECTIONS & detections)
{
	...
	for (Track & track : tracks) {
		...
		FEATURESS t = FEATURESS(0, SORT_SHAPE);
		track.features = t;
	}
...
void tracker::_initiate_track(const TrackBoxConvert & detection)
{
...
{
	FEATURESS features(detection_indices.size(), SORT_SHAPE);
	int pos = 0;
