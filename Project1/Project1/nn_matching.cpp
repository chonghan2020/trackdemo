  ...
  for (TRACKER_DATA & data:tid_feats) {
        ...
        if (samples.find(track_id) != samples.end()) {  //append
            ...
            if (newSize <= this->budget) {
                FEATURESS newSampleFeatures(newSize, SORT_SHAPE);
                newSampleFeatures.block(0, 0, oldSize, SORT_SHAPE) = samples[track_id];
                newSampleFeatures.block(oldSize, 0, addSize, SORT_SHAPE) = newFeatOne;
                samples[track_id] = newSampleFeatures;
            } else {
                if (oldSize < this->budget) {   //original space is not enough;
                    FEATURESS newSampleFeatures(this->budget, SORT_SHAPE);
                    if (addSize >= this->budget) {
                        newSampleFeatures = newFeatOne.block(0, 0, this->budget, SORT_SHAPE);
                    } else {
                        newSampleFeatures.block(0, 0, this->budget - addSize, SORT_SHAPE) =
                            samples[track_id].block(addSize - 1, 0, this->budget - addSize, SORT_SHAPE).eval();
                        newSampleFeatures.block(this->budget - addSize, 0, addSize, SORT_SHAPE) = newFeatOne;
                    }
                    samples[track_id] = newSampleFeatures;
                } else {        //original space is ok;
                    if (addSize >= this->budget) {
                        samples[track_id] = newFeatOne.block(0, 0, this->budget, SORT_SHAPE);
                    } else {
                        samples[track_id].block(0, 0, this->budget - addSize, SORT_SHAPE) =
                            samples[track_id].block(addSize - 1, 0, this->budget - addSize, SORT_SHAPE).eval();
                        samples[track_id].block(this->budget - addSize, 0, addSize, SORT_SHAPE) = newFeatOne;
                    }
                }
            }
